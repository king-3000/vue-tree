...
:::demo
```html
<devops-tree :data="data" :props="defaultProps" @node-click="handleNodeClick"></devops-tree>
      ...
      renderContent(h, { node, data, store }) {
        return (
          <span class="custom-tree-node">
            <span>{node.label}</span>
            <span>
              <button size="mini" type="text" on-click={ () => this.append(data) }>Append</button>
              <button size="mini" type="text" on-click={ () => this.remove(node, data) }>Delete</button>
            </span>
          </span>);
      }
    }
